...
const promise: Promise<string> = new Promise((resolve, reject) => {  // const promise: Promise<unknown>
    setTimeout(()=>{
...
const mergeObj = merge({name: 'Max'}, {age: 30}) as {name: string, age: string};    // 굳이 이렇게 형변환? → 제네릭 사용
// mergeObj.name;   접근 X → objA/B의 타입을 객체로 입력하여 객체를 반환하는 것으로 추론

function merge2<T, U>(objC: T, objD: U){ // T와 U는 일반적으로 제네릭 타입을 사용할 때 쓰는 식별자
