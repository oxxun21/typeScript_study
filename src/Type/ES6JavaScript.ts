...
function addVar (a: number, b: number) {
    var result;
    result = a + b;
    return result;
}
...
function addVar2 (a: number, b: number) {
    result = a + b;
    return result;
}
...
const addArrow2 = (a: number, b: number = 1) => a + b;
...
const copiedPerson1 = { ...person1 };   // 객체가 필요한 곳 어디서든 사용 가능

// 나머지 매개변수
// const add3 = (a, b, c, d) => {};  유연한 작업 불가
const add3 = (...numbers: number[]) => {      // 이름은 맘대로, 들어오는 매개변수 또는 일반적으로 들어오는 쉼표로 분리된 값의 목록을 배열로 병합
    let result = 0;
