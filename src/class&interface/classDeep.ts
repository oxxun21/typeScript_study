abstract class DepartmentData {
    ...
    // describe(this: DepartmentData){
    //     // console.log('Department: ' + this.name);
    //     console.log(`Department (${this.id}): ${this.name}`)
    // }
    abstract describe(this: DepartmentData): void;  // 중괄호 제거 후 추가 보유해야하는 타입 반환
...
class ITDepartment extends DepartmentData {     // extends 오른쪽은 상속 받을 class
    admins: string[];
...
class AccountingDepartment extends DepartmentData {  
    ...
    // 세터: 속성 값 설정
    set mostRecentReport(value: string){    // 사용자가 전달한 값인 인수를 취함
        if (!value){
