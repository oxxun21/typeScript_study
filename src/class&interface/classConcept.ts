...
class Department {
    // 클래스 필드
    name: string;   // 키 이름만 정의, 클래스를 기반으로 만들 객체를 입력하고 마지막에 키가 갖게 될 값의 타입을 정의
...
class Department2 {
    ...
    describe(this: Department2) {    // this 추가, 값을 전달하지 않고도 describe() 호출 가능, ts는 this가 무엇으로 참조되어야 하는지 인식
